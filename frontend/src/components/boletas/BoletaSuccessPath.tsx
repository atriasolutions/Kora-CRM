import { ArrowLeft, ArrowRight, Check, ChevronRight, Pause, Receipt } from 'lucide-react'
...
import {
  ...
  BOLETA_OFF_ROUTE_ENTRY_MAIN_STAGES,
  boletaStageDisplayName,
  getBoletaMainLineTransitions,
  getBoletaOffRouteTransitions,
  getBoletaPreviousMainStage,
  getBoletaResumeMainStage,
  inferBoletaPausedFromMainStage,
  isBoletaMainLineStage,
  isBoletaOffRouteStage,
  type BoletaJourneyStage,
  type BoletaStatusHistoryEntry,
} from '@/lib/boleta-journey'
import { cn } from '@/lib/utils'

type BoletaSuccessPathProps = {
  currentStage: BoletaJourneyStage
  history?: BoletaStatusHistoryEntry[]
  readOnly?: boolean
  onStageChange?: (stage: BoletaJourneyStage) => void
  className?: string
}
...
function shortMainLabel(stage: string) {
  return boletaStageDisplayName(stage as BoletaJourneyStage)
}

export function BoletaSuccessPath({
  currentStage,
  history = [],
  readOnly = false,
  onStageChange,
  className,
}: BoletaSuccessPathProps) {
  const historyByStage = useMemo(
    () => new Map(history.map((h) => [h.status, h])),
    [history],
  )

  const onOffRoute = isBoletaOffRouteStage(currentStage)
  const pausedFrom = inferBoletaPausedFromMainStage(currentStage, history)
  const pausedFromIndex = BOLETA_JOURNEY_MAIN_LINE.indexOf(pausedFrom)
  const mainIndex = BOLETA_JOURNEY_MAIN_LINE.indexOf(
    currentStage as (typeof BOLETA_JOURNEY_MAIN_LINE)[number],
  )
  ...
  const progressPct =
    BOLETA_JOURNEY_MAIN_LINE.length > 1
      ? (progressIndex / (BOLETA_JOURNEY_MAIN_LINE.length - 1)) * 100
      : 0

  const resumeStage = onOffRoute ? getBoletaResumeMainStage(currentStage, history) : null
  const mainTransitions = getBoletaMainLineTransitions(currentStage)
  const offRouteTransitions = getBoletaOffRouteTransitions(currentStage)
  const resumeOnly =
    onOffRoute && mainTransitions.length === 1 && offRouteTransitions.length === 0
  const previousMain =
    isBoletaMainLineStage(currentStage) ? getBoletaPreviousMainStage(currentStage) : null
  const canGoBack =
    previousMain !== null && mainTransitions.includes(previousMain)

  const mainStepState = (
    stage: (typeof BOLETA_JOURNEY_MAIN_LINE)[number],
    index: number,
  ): MainStepState => {
  ...
  return (
    <section
      ...
      aria-label="Ruta del éxito de la boleta"
    >
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border bg-muted/20 px-4 py-3 sm:px-5">
              ...
              {onOffRoute ? (
                ...
                <>
                  ...
                  <span className="font-medium text-foreground">
                    {boletaStageDisplayName(currentStage)}
                  </span>
                </>
              )}
            </p>
        ...
        {!readOnly && onStageChange ? (
          <div className="flex flex-wrap items-center gap-2">
            ...
            {currentStage === 'Borrador' && mainTransitions.includes('Emitida') ? (
              <Button
                type="button"
                size="sm"
                className="h-8 gap-1 text-xs"
                onClick={() => onStageChange('Emitida')}
              >
                Emitir comprobante
            ...
            <DropdownMenu>
              ...
              >
                <DropdownMenuLabel>
                  Desde «{boletaStageDisplayName(currentStage)}»
                </DropdownMenuLabel>
                <DropdownMenuSeparator />
                {resumeOnly && mainTransitions[0] ? (
                  <DropdownMenuItem onSelect={() => onStageChange(mainTransitions[0]!)}>
                    Reanudar en {boletaStageDisplayName(mainTransitions[0]!)}
                  </DropdownMenuItem>
                ) : null}
                {!resumeOnly && mainTransitions.length > 0 ? (
                  <>
                    <DropdownMenuLabel className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">
                      Flujo principal
                    </DropdownMenuLabel>
                    {mainTransitions.map((stage) => (
                      <DropdownMenuItem key={stage} onSelect={() => onStageChange(stage)}>
                        {boletaStageDisplayName(stage)}
                      </DropdownMenuItem>
                    ))}
                  </>
                ) : null}
                {!resumeOnly && offRouteTransitions.length > 0 ? (
                  <>
                    <DropdownMenuSeparator />
                    <DropdownMenuLabel className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">
                      Fuera de la ruta
                    </DropdownMenuLabel>
                    {offRouteTransitions.map((stage) => (
                      <DropdownMenuItem key={stage} onSelect={() => onStageChange(stage)}>
                        {boletaStageDisplayName(stage)}
                      </DropdownMenuItem>
                    ))}
      ...
      <div className="space-y-4 p-4 sm:p-5">
        <div className="relative mx-auto max-w-4xl">
          ...
          <ol className="relative flex justify-between gap-0.5">
            {BOLETA_JOURNEY_MAIN_LINE.map((stage, index) => {
        ...
        >
            ...
            <div className="flex items-start gap-2.5">
              ...
              <div>
                ...
                <p className="mt-0.5 text-xs text-muted-foreground">
                  Anulación del comprobante. Disponible desde{' '}
                  <span className="font-medium text-foreground">
                    {BOLETA_OFF_ROUTE_ENTRY_MAIN_STAGES.map(shortMainLabel).join(', ')}
                  </span>
          ...
          >
            {BOLETA_JOURNEY_OFF_ROUTE.map((stage) => {
