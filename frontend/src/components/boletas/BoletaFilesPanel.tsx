...
import type { BoletaFile } from '@/lib/boleta-files'
import {
  canPreviewBoletaFile,
  boletaFileFromUpload,
  boletaFileIconKind,
  getBoletaFilePreviewUrl,
  isDemoBoletaFilePreview,
  validateBoletaFilesForUpload,
} from '@/lib/boleta-files'
import { cn } from '@/lib/utils'

type BoletaFilesPanelProps = {
  authorName: string
  files: BoletaFile[]
  disabled?: boolean
  onFilesChange: (files: BoletaFile[]) => void
}
...
function FileTypeIcon({
  ...
  kind: ReturnType<typeof boletaFileIconKind>
  className?: string
}) {
...
export function BoletaFilesPanel({
  authorName,
  files,
  disabled = false,
  onFilesChange,
}: BoletaFilesPanelProps) {
  const inputId = useId()
  const inputRef = useRef<HTMLInputElement>(null)
  const [dragOver, setDragOver] = useState(false)
  const [uploading, setUploading] = useState(false)
  const [previewFile, setPreviewFile] = useState<BoletaFile | null>(null)

  const previewUrl = previewFile ? getBoletaFilePreviewUrl(previewFile) : null
  ...
  const addFiles = useCallback(
    async (fileList: FileList | null) => {
      ...
      const validation = validateBoletaFilesForUpload(files, incoming)
      if (validation) {
      ...
      setUploading(true)
      try {
        const uploaded: BoletaFile[] = []
        for (const file of incoming) {
          uploaded.push(await boletaFileFromUpload(file, authorName))
        }
      ...
      <CardContent className="space-y-4">
        ...
        {files.length === 0 ? (
          <p className="text-center text-sm text-muted-foreground">
            Aún no hay archivos asociados a este esta boleta.
          </p>
        ) : (
          <ul className="divide-y divide-border rounded-lg border border-border">
            {files.map((file) => {
              const kind = boletaFileIconKind(file.mimeType, file.name)
              const canDownload = Boolean(file.dataUrl)
              const canPreview = canPreviewBoletaFile(file)
      ...
      >
        <DialogContent className="flex max-h-[92vh] flex-col gap-0 overflow-hidden p-0 sm:max-w-4xl">
          <DialogHeader className="border-b border-border px-6 py-4">
            ...
            <DialogDescription>
              {previewFile && isDemoBoletaFilePreview(previewFile)
