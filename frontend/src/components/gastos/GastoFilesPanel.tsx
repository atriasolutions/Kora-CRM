...
import type { ExpenseFile } from '@/lib/expense-files'
import {
  canPreviewExpenseFile,
  expenseFileFromUpload,
  expenseFileIconKind,
  getExpenseFilePreviewUrl,
  isDemoExpenseFilePreview,
  validateExpenseFilesForUpload,
} from '@/lib/expense-files'
import { cn } from '@/lib/utils'

type ExpenseFilesPanelProps = {
  authorName: string
  files: ExpenseFile[]
  disabled?: boolean
  onFilesChange: (files: ExpenseFile[]) => void
}
...
function FileTypeIcon({
  ...
  kind: ReturnType<typeof expenseFileIconKind>
  className?: string
}) {
...
export function ExpenseFilesPanel({
  ...
  disabled = false,
  onFilesChange,
}: ExpenseFilesPanelProps) {
  const inputId = useId()
  const inputRef = useRef<HTMLInputElement>(null)
  const [dragOver, setDragOver] = useState(false)
  const [uploading, setUploading] = useState(false)
  const [previewFile, setPreviewFile] = useState<ExpenseFile | null>(null)

  const previewUrl = previewFile ? getExpenseFilePreviewUrl(previewFile) : null
  ...
  const addFiles = useCallback(
    async (fileList: FileList | null) => {
      ...
      const validation = validateExpenseFilesForUpload(files, incoming)
      if (validation) {
      ...
      try {
        ...
        for (const file of incoming) {
          uploaded.push(await expenseFileFromUpload(file, authorName))
        }
      ...
      <CardContent className="space-y-4">
        ...
        {files.length === 0 ? (
          <p className="text-center text-sm text-muted-foreground">
            Aún no hay archivos asociados a este este gasto.
          </p>
        ) : (
          <ul className="divide-y divide-border rounded-lg border border-border">
            {files.map((file) => {
              const kind = expenseFileIconKind(file.mimeType, file.name)
              const canDownload = Boolean(file.dataUrl)
              const canPreview = canPreviewExpenseFile(file)
      ...
      >
        <DialogContent className="flex max-h-[92vh] flex-col gap-0 overflow-hidden p-0 sm:max-w-4xl">
          <DialogHeader className="border-b border-border px-6 py-4">
            ...
            <DialogDescription>
              {previewFile && isDemoExpenseFilePreview(previewFile)
