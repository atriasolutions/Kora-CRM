...
import { parseCompaniesCsv, type CreateCompanyFormValues } from '@/lib/company-create'
import { cn } from '@/lib/utils'

type ImportCompaniesDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  onImport: (rows: CreateCompanyFormValues[]) => void
}
...
export function ImportCompaniesDialog({
  ...
  const [parsedRows, setParsedRows] = useState<CreateCompanyFormValues[]>([])
  ...
  const handleFile = async (file: File) => {
    ...
    const result = parseCompaniesCsv(text)
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            CSV con columnas: nombre, industria, ciudad, empleados, responsable, etapa
            (Prospecto, Cliente, Proveedor), estado (Activa, Inactiva).
          </DialogDescription>
        </DialogHeader>
        ...
        >
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              También acepta filas sin cabecera: nombre, industria, ciudad…
            </p>
          </div>
        </button>
          ...
          <div className="flex items-center gap-3 rounded-lg border border-border bg-muted/30 px-4 py-3 text-sm">
            ...
            <div>
              <p className="font-medium text-foreground">
                {previewCount} empresa{previewCount === 1 ? '' : 's'} lista
                {previewCount === 1 ? '' : 's'} para importar
              </p>
              {skipped > 0 ? (
                <p className="text-xs text-muted-foreground">
                  {skipped} fila{skipped === 1 ? '' : 's'} omitida{skipped === 1 ? '' : 's'}
                </p>
              ) : null}
            </div>
          </div>
