...
import { CompaniesFiltersMenu } from '@/components/companies/CompaniesFiltersMenu'
...
import {
  COMPANY_LIST_SCOPE_OPTIONS,
  COMPANY_SCOPE_SHORT_LABELS,
...
export type CompaniesViewId = StandardModuleViewId

type CompaniesModuleHeaderProps = {
  view: CompaniesViewId
  onViewChange: (view: CompaniesViewId) => void
  query: string
  ...
  onListScopeChange?: (scope: CompanyListScope) => void
  archivedCount?: number
  toolbarEnd?: ReactNode
}

export function CompaniesModuleHeader({
  view,
  onViewChange,
  query,
  onQueryChange,
  ...
  archivedCount = 0,
  toolbarEnd,
}: CompaniesModuleHeaderProps) {
  const { canCreate } = useModulePermissions('empresas')
  const activeFilters = countActiveCompanyFilters(filters)
      ...
      <div className="flex items-start justify-between gap-3 md:items-center">
        <div className="min-w-0">
          ...
          <p className="mt-1 hidden text-sm text-muted-foreground md:block">
            {view === 'archivados'
              ? 'Papelera de reciclaje: restaura o elimina empresas archivadas.'
              : 'Cuentas y organizaciones vinculadas a tu pipeline.'}
            {activeFilters > 0
        ...
        <div className="flex shrink-0 items-center gap-2">
          {view !== 'archivados' ? (
            <CompaniesFiltersMenu filters={filters} onFiltersChange={onFiltersChange} />
          ) : null}
      ...
      <ModuleListToolbarBar
        viewSwitcher={
          <ModuleViewSwitcher
            ...
            tablistAriaLabel="Vista de empresas"
        ...
        scopeSwitcher={
          showListScope ? (
            <ModuleListScopeSwitcher
              ...
              options={COMPANY_LIST_SCOPE_OPTIONS}
              shortLabels={COMPANY_SCOPE_SHORT_LABELS}
              showLabel
            />
          ) : undefined
        }
        search={
          <ModuleSearchField
            ...
            placeholder={
              view === 'archivados' ? 'Buscar en archivados…' : 'Buscar empresas…'
            }
