...
import type { CompanyFile } from '@/lib/company-files'
import {
  canPreviewCompanyFile,
  companyFileFromUpload,
  fileIconKind,
  getCompanyFilePreviewUrl,
  isDemoCompanyFilePreview,
  validateCompanyFilesForUpload,
...
export function CompanyFilesPanel({
  ...
  const previewUrl = previewFile ? getCompanyFilePreviewUrl(previewFile) : null
  ...
  const addFiles = useCallback(
    async (fileList: FileList | null) => {
      ...
      const validation = validateCompanyFilesForUpload(files, incoming)
      if (validation) {
      ...
      try {
        ...
        for (const file of incoming) {
          uploaded.push(await companyFileFromUpload(file, authorName))
        }
      ...
      <CardContent className="space-y-4">
        ...
        {files.length === 0 ? (
          <p className="text-center text-sm text-muted-foreground">
            Aún no hay archivos asociados a esta empresa.
          </p>
        ) : (
          <ul className="divide-y divide-border rounded-lg border border-border">
            {files.map((file) => {
              ...
              const canPreview = canPreviewCompanyFile(file)
      ...
      >
        <DialogContent className="flex max-h-[92vh] flex-col gap-0 overflow-hidden p-0 sm:max-w-4xl">
          <DialogHeader className="border-b border-border px-6 py-4">
            ...
            <DialogDescription>
              {previewFile && isDemoCompanyFilePreview(previewFile)
