...
import type { ContactFile } from '@/lib/contact-files'
import {
  canPreviewContactFile,
  contactFileFromUpload,
  fileIconKind,
  getContactFilePreviewUrl,
  isDemoContactFilePreview,
  validateFilesForUpload,
} from '@/lib/contact-files'
import { cn } from '@/lib/utils'

type ContactFilesPanelProps = {
  authorName: string
  files: ContactFile[]
  disabled?: boolean
  onFilesChange: (files: ContactFile[]) => void
}
...
export function ContactFilesPanel({
  ...
  const previewUrl = previewFile ? getContactFilePreviewUrl(previewFile) : null
  ...
  const addFiles = useCallback(
    async (fileList: FileList | null) => {
      ...
      const validation = validateFilesForUpload(files, incoming)
      if (validation) {
      ...
      setUploading(true)
      try {
        const uploaded: ContactFile[] = []
        for (const file of incoming) {
          uploaded.push(await contactFileFromUpload(file, authorName))
        }
      ...
      <CardContent className="space-y-4">
        ...
        {files.length === 0 ? (
          <p className="text-center text-sm text-muted-foreground">
            Aún no hay archivos asociados a este contacto.
          </p>
        ) : (
          <ul className="divide-y divide-border rounded-lg border border-border">
            {files.map((file) => {
              ...
              const canPreview = canPreviewContactFile(file)
      ...
      >
        <DialogContent className="flex max-h-[92vh] flex-col gap-0 overflow-hidden p-0 sm:max-w-4xl">
          <DialogHeader className="border-b border-border px-6 py-4">
            ...
            <DialogDescription>
              {previewFile && isDemoContactFilePreview(previewFile)
