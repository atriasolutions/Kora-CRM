...
export function ProductsSegmentsView({
  ...
  const searched = useMemo(() => {
    let result = filterProducts(allProducts, query, (p) =>
    ...
    return result
  }, [allProducts, query, filters, isArchived, listScope, recentIds])
  ...
  const counts = useMemo(
    () =>
      Object.fromEntries(
        productSegments.map((s) => [s.id, countSegmentMatches(searched, s)]),
      ),
    [searched],
  )
