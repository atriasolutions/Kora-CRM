...
import type { ArchivedProductEntry } from '@/contexts/products-registry-context'
...
import {
  ...
  PRODUCT_PERMANENT_DELETE_INVENTORY_WARNING,
  daysUntilPurge,
...
type ProductsArchivedViewProps = {
  query: string
}
...
function matchesQuery(entry: ArchivedProductEntry, q: string): boolean {
  if (!q) return true
  const { product } = entry
  return (
...
export function ProductsArchivedView({ query }: ProductsArchivedViewProps) {
  const {
    archivedProducts,
    restoreProduct,
    restoreProducts,
  ...
  const { canDelete: canManageArchive } = useModulePermissions('productos')
  ...
  const filtered = useMemo(
    () => archivedProducts.filter((e) => matchesQuery(e, q)),
    [archivedProducts, q],
  )
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (confirm.type === 'restore') {
        if (count === 1) await restoreProduct(confirm.ids[0]!)
        else await restoreProducts(confirm.ids)
        toast.success(
          count === 1
            ? 'Producto restaurado correctamente.'
            : `${count} productos restaurados.`,
        )
      } else {
        if (count === 1) await permanentlyDeleteProduct(confirm.ids[0]!)
        else await permanentlyDeleteProducts(confirm.ids)
        toast.success(
          count === 1
            ? 'Producto eliminado definitivamente.'
            : `${count} productos eliminados definitivamente.`,
        )
      }
      setSelected(new Set())
      setConfirm(null)
      ...
      >
        ...
        <div className="space-y-1">
          ...
          <p className="text-amber-900/90 dark:text-amber-100/90">
            Los productos archivados permanecen aquí durante{' '}
            <strong>{PRODUCT_ARCHIVE_RETENTION_DAYS} días</strong>. Después se eliminan
            ...
            <DialogTitle>
              {confirm?.type === 'restore'
                ? confirm.ids.length === 1
                  ? 'Restaurar producto'
                  : `Restaurar ${confirm.ids.length} productos`
                : confirm?.ids.length === 1
                  ? 'Eliminar definitivamente'
                  : `Eliminar ${confirm?.ids.length} productos`}
            </DialogTitle>
            <DialogDescription className="space-y-2">
              {confirm?.type === 'restore' ? (
                'El producto volverá a aparecer en tus listados activos.'
              ) : (
                <>
                  <span>
                    {confirm?.ids.length === 1
                      ? 'Se borrarán todos los datos de este producto.'
                      : `Se borrarán todos los datos de ${confirm?.ids.length} productos.`}
                  </span>
