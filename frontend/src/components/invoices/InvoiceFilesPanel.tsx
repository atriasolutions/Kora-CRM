...
import type { InvoiceFile } from '@/lib/invoice-files'
import {
  canPreviewInvoiceFile,
  invoiceFileFromUpload,
  invoiceFileIconKind,
  getInvoiceFilePreviewUrl,
  isDemoInvoiceFilePreview,
  validateInvoiceFilesForUpload,
} from '@/lib/invoice-files'
import { cn } from '@/lib/utils'

type InvoiceFilesPanelProps = {
  authorName: string
  files: InvoiceFile[]
  disabled?: boolean
...
function FileTypeIcon({
  ...
  kind: ReturnType<typeof invoiceFileIconKind>
  className?: string
}) {
...
export function InvoiceFilesPanel({
  ...
  const previewUrl = previewFile ? getInvoiceFilePreviewUrl(previewFile) : null
  ...
  const addFiles = useCallback(
    async (fileList: FileList | null) => {
      ...
      const validation = validateInvoiceFilesForUpload(files, incoming)
      if (validation) {
      ...
      setUploading(true)
      try {
        const uploaded: InvoiceFile[] = []
        for (const file of incoming) {
          uploaded.push(await invoiceFileFromUpload(file, authorName))
        }
      ...
      <CardContent className="space-y-4">
        ...
        {files.length === 0 ? (
          <p className="text-center text-sm text-muted-foreground">
            Aún no hay archivos asociados a este esta factura.
          </p>
        ) : (
          <ul className="divide-y divide-border rounded-lg border border-border">
            {files.map((file) => {
              const kind = invoiceFileIconKind(file.mimeType, file.name)
              const canDownload = Boolean(file.dataUrl)
              const canPreview = canPreviewInvoiceFile(file)
      ...
      >
        <DialogContent className="flex max-h-[92vh] flex-col gap-0 overflow-hidden p-0 sm:max-w-4xl">
          <DialogHeader className="border-b border-border px-6 py-4">
            ...
            <DialogDescription>
              {previewFile && isDemoInvoiceFilePreview(previewFile)
