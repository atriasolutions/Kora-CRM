...
type InventorySegmentsViewProps = {
  query: string
  filters: InventoryFilters
  items?: InventoryListItem[]
}

export function InventorySegmentsView({
  query,
  filters,
  items: itemsProp,
}: InventorySegmentsViewProps) {
  const navigate = useNavigate()
  const [activeSegmentId, setActiveSegmentId] = useState(inventorySegments[0]!.id)

  const allItems = useMemo(
    () => itemsProp ?? mergeWithDemoDataset([], getInventoryBoardDataset()),
    [itemsProp],
  )
  const searched = useMemo(
    () => filterInventory(allItems, query, (row) => inventoryRowMatchesFilters(row, filters)),
    [allItems, query, filters],
  )

  const activeSegment = useMemo(
    () => inventorySegments.find((s) => s.id === activeSegmentId) ?? inventorySegments[0]!,
    [activeSegmentId],
  )
  ...
  const counts = useMemo(
    () =>
      Object.fromEntries(
        inventorySegments.map((s) => [s.id, countInventorySegmentMatches(searched, s)]),
      ),
    [searched],
  )
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="block text-xs text-muted-foreground">
                      {item.sku} · {item.location} · {item.quantity}
                    </span>
                  </span>
                  <span className="hidden shrink-0 flex-wrap justify-end gap-2 sm:flex">
                    ...
                    <Badge variant="secondary">Mín. {item.minStock}</Badge>
                  </span>
