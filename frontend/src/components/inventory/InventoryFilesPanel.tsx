...
import type { InventoryFile } from '@/lib/inventory-files'
import {
  canPreviewInventoryFile,
  inventoryFileFromUpload,
  fileIconKind,
  getInventoryFilePreviewUrl,
  isDemoInventoryFilePreview,
  validateInventoryFilesForUpload,
} from '@/lib/inventory-files'
import { cn } from '@/lib/utils'

type InventoryFilesPanelProps = {
  authorName: string
  files: InventoryFile[]
  disabled?: boolean
  onFilesChange: (files: InventoryFile[]) => void
}
...
export function InventoryFilesPanel({
  authorName,
  files,
  disabled = false,
  onFilesChange,
}: InventoryFilesPanelProps) {
  const inputId = useId()
  ...
  const previewUrl = previewFile ? getInventoryFilePreviewUrl(previewFile) : null
  ...
  const addFiles = useCallback(
    async (fileList: FileList | null) => {
      ...
      const validation = validateInventoryFilesForUpload(files, incoming)
      if (validation) {
      ...
      setUploading(true)
      try {
        const uploaded: InventoryFile[] = []
        for (const file of incoming) {
          uploaded.push(await inventoryFileFromUpload(file, authorName))
        }
      ...
      <CardContent className="space-y-4">
        ...
        {files.length === 0 ? (
          <p className="text-center text-sm text-muted-foreground">
            Aún no hay archivos asociados a este ítem de inventario.
          </p>
        ) : (
          <ul className="divide-y divide-border rounded-lg border border-border">
            {files.map((file) => {
              ...
              const canPreview = canPreviewInventoryFile(file)
      ...
      >
        <DialogContent className="flex max-h-[92vh] flex-col gap-0 overflow-hidden p-0 sm:max-w-4xl">
          <DialogHeader className="border-b border-border px-6 py-4">
            ...
            <DialogDescription>
              {previewFile && isDemoInventoryFilePreview(previewFile)
