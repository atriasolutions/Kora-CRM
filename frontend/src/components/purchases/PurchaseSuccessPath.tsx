import { ArrowLeft, ArrowRight, Check, ChevronRight, Pause, ShoppingCart } from 'lucide-react'
...
import type { PurchaseStageHistory } from '@/lib/purchase-journey'
import {
  PURCHASE_JOURNEY_MAIN_LINE,
  PURCHASE_JOURNEY_OFF_ROUTE,
  PURCHASE_OFF_ROUTE_ENTRY_MAIN_STAGES,
...
type PurchaseSuccessPathProps = {
  currentStage: PurchaseJourneyStage
  history?: PurchaseStageHistory[]
  readOnly?: boolean
  onStageChange?: (stage: PurchaseJourneyStage) => void
  className?: string
}
...
function shortOffRouteLabel(stage: string) {
  if (stage === 'En espera proveedor') return 'Espera proveedor'
  return stage
}

export function PurchaseSuccessPath({
  currentStage,
  history = [],
  readOnly = false,
  onStageChange,
  className,
}: PurchaseSuccessPathProps) {
  ...
  const pausedFromIndex = PURCHASE_JOURNEY_MAIN_LINE.indexOf(pausedFrom)
  const mainIndex = PURCHASE_JOURNEY_MAIN_LINE.indexOf(
    currentStage as (typeof PURCHASE_JOURNEY_MAIN_LINE)[number],
  )
  ...
  const progressPct =
    PURCHASE_JOURNEY_MAIN_LINE.length > 1
      ? (progressIndex / (PURCHASE_JOURNEY_MAIN_LINE.length - 1)) * 100
      : 0
  ...
  const mainStepState = (
    stage: (typeof PURCHASE_JOURNEY_MAIN_LINE)[number],
    index: number,
  ): MainStepState => {
  ...
  return (
    <section
      ...
      aria-label="Ruta del éxito de la compra"
    >
      ...
      <div className="space-y-4 p-4 sm:p-5">
        <div className="relative mx-auto max-w-4xl">
          ...
          <ol className="relative flex justify-between gap-0.5">
            {PURCHASE_JOURNEY_MAIN_LINE.map((stage, index) => {
        ...
        {showOffRoutePanel ? (
          ...
          >
              ...
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-muted-foreground">
                    Espera del proveedor o cancelación. Disponible desde{' '}
                    <span className="font-medium text-foreground">
                      {PURCHASE_OFF_ROUTE_ENTRY_MAIN_STAGES.map(shortMainLabel).join(', ')}
                    </span>
            ...
            >
              {PURCHASE_JOURNEY_OFF_ROUTE.map((stage) => {
            ...
            {!onOffRoute && offRouteTransitions.length === 0 ? (
              <p className="mt-3 text-xs text-muted-foreground">
                Con la OC en estado{' '}
                <strong className="font-medium text-foreground">Emitida</strong> o{' '}
                <strong className="font-medium text-foreground">Confirmada</strong> puedes registrar
                ingresos de stock desde el detalle.
              </p>
            ) : null}
          </aside>
        ) : null}
