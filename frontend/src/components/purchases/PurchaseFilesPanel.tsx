...
import type { PurchaseFile } from '@/lib/purchase-files'
import {
  canPreviewPurchaseFile,
  purchaseFileFromUpload,
  fileIconKind,
  getPurchaseFilePreviewUrl,
  isDemoPurchaseFilePreview,
  validatePurchaseFilesForUpload,
} from '@/lib/purchase-files'
import { cn } from '@/lib/utils'

type PurchaseFilesPanelProps = {
  authorName: string
  files: PurchaseFile[]
  disabled?: boolean
  onFilesChange: (files: PurchaseFile[]) => void
}
...
export function PurchaseFilesPanel({
  authorName,
  files,
  disabled = false,
  onFilesChange,
}: PurchaseFilesPanelProps) {
  const inputId = useId()
  ...
  const previewUrl = previewFile ? getPurchaseFilePreviewUrl(previewFile) : null
  ...
  const addFiles = useCallback(
    async (fileList: FileList | null) => {
      ...
      const validation = validatePurchaseFilesForUpload(files, incoming)
      if (validation) {
      ...
      setUploading(true)
      try {
        const uploaded: PurchaseFile[] = []
        for (const file of incoming) {
          uploaded.push(await purchaseFileFromUpload(file, authorName))
        }
      ...
      <CardContent className="space-y-4">
        ...
        {files.length === 0 ? (
          <p className="text-center text-sm text-muted-foreground">
            Aún no hay archivos asociados a esta compra.
          </p>
        ) : (
          <ul className="divide-y divide-border rounded-lg border border-border">
            {files.map((file) => {
              ...
              const canPreview = canPreviewPurchaseFile(file)
      ...
      >
        <DialogContent className="flex max-h-[92vh] flex-col gap-0 overflow-hidden p-0 sm:max-w-4xl">
          <DialogHeader className="border-b border-border px-6 py-4">
            ...
            <DialogDescription>
              {previewFile && isDemoPurchaseFilePreview(previewFile)
