...
import type { ArchivedPurchaseEntry } from '@/contexts/purchases-registry-context'
...
import {
  PURCHASE_ARCHIVE_RETENTION_DAYS,
  daysUntilPurge,
...
type PurchasesArchivedViewProps = {
  query: string
}
...
function matchesQuery(entry: ArchivedPurchaseEntry, q: string): boolean {
  if (!q) return true
  const { purchase } = entry
  return (
    purchase.reference.toLowerCase().includes(q) ||
    purchase.supplier.toLowerCase().includes(q) ||
    purchase.owner.toLowerCase().includes(q)
  )
}

export function PurchasesArchivedView({ query }: PurchasesArchivedViewProps) {
  const {
    archivedPurchases,
    restorePurchase,
    restorePurchases,
  ...
  const { canDelete: canManageArchive } = useModulePermissions('compras')
  ...
  const filtered = useMemo(
    () => archivedPurchases.filter((e) => matchesQuery(e, q)),
    [archivedPurchases, q],
  )
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (confirm.type === 'restore') {
        if (count === 1) await restorePurchase(confirm.ids[0]!)
        else await restorePurchases(confirm.ids)
        toast.success(
          count === 1
            ? 'Compra restaurada correctamente.'
            : `${count} compras restauradas.`,
        )
      } else {
        if (count === 1) await permanentlyDeletePurchase(confirm.ids[0]!)
        else await permanentlyDeletePurchases(confirm.ids)
        toast.success(
          count === 1
            ? 'Compra eliminada definitivamente.'
            : `${count} compras eliminadas definitivamente.`,
        )
      }
      setSelected(new Set())
      setConfirm(null)
      ...
      >
        ...
        <div className="space-y-1">
          ...
          <p className="text-amber-900/90 dark:text-amber-100/90">
            Los purchaseos archivados permanecen aquí durante{' '}
            <strong>{PURCHASE_ARCHIVE_RETENTION_DAYS} días</strong>. Después se eliminan
          ...
          <table className="w-full min-w-[720px] border-collapse text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/30 text-left">
                ...
                <th className="px-4 py-3 font-semibold text-foreground">Purchaseo</th>
                ...
                <tr>
                  ...
                  >
                    {archivedPurchases.length === 0
                      ? 'No hay purchaseos en la papelera.'
                  ...
                  return (
                    ...
                    >
                      <td className="px-4 py-3">
                        <input
                          aria-label={`Seleccionar ${purchase.reference}`}
                        ...
                        <div className="flex items-center gap-3">
                          <div className="min-w-0">
                            ...
                            <p className="truncate text-xs text-muted-foreground">
                              {purchase.supplier} · {purchase.amount}
                            </p>
                          </div>
            ...
            <DialogTitle>
              {confirm?.type === 'restore'
                ? confirm.ids.length === 1
                  ? 'Restaurar purchaseo'
                  : `Restaurar ${confirm.ids.length} purchaseos`
                : confirm?.ids.length === 1
                  ? 'Eliminar definitivamente'
                  : `Eliminar ${confirm?.ids.length} purchaseos`}
            </DialogTitle>
            <DialogDescription>
              {confirm?.type === 'restore'
                ? 'El purchaseo volverá a aparecer en tus listados activos.'
                : 'Esta acción no se puede deshacer. Se borrarán los datos guardados localmente de este purchaseo.'}
            </DialogDescription>
          </DialogHeader>
