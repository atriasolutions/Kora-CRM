...
import type { ArchivedProjectEntry } from '@/contexts/projects-registry-context'
...
type ProjectsArchivedViewProps = {
  query: string
}
...
function matchesQuery(entry: ArchivedProjectEntry, q: string): boolean {
  if (!q) return true
  const { project } = entry
  return (
...
export function ProjectsArchivedView({ query }: ProjectsArchivedViewProps) {
  const {
    archivedProjects,
    restoreProject,
    restoreProjects,
  ...
  const { canDelete: canManageArchive } = useModulePermissions('proyectos')
  ...
  const filtered = useMemo(
    () => archivedProjects.filter((e) => matchesQuery(e, q)),
    [archivedProjects, q],
  )
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (confirm.type === 'restore') {
        if (count === 1) await restoreProject(confirm.ids[0]!)
        else await restoreProjects(confirm.ids)
        toast.success(
          count === 1
            ? 'Proyecto restaurado correctamente.'
            : `${count} proyectos restaurados.`,
        )
      } else {
        if (count === 1) await permanentlyDeleteProject(confirm.ids[0]!)
        else await permanentlyDeleteProjects(confirm.ids)
        toast.success(
          count === 1
            ? 'Proyecto eliminado definitivamente.'
            : `${count} proyectos eliminados definitivamente.`,
        )
      }
      setSelected(new Set())
      setConfirm(null)
      ...
      >
        ...
        <div className="space-y-1">
          ...
          <p className="text-amber-900/90 dark:text-amber-100/90">
            Los projectos archivados permanecen aquí durante{' '}
            <strong>{PROJECT_ARCHIVE_RETENTION_DAYS} días</strong>. Después se eliminan
          ...
          <table className="w-full min-w-[720px] border-collapse text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/30 text-left">
                ...
                <th className="px-4 py-3 font-semibold text-foreground">Projecto</th>
                ...
                <tr>
                  ...
                  >
                    {archivedProjects.length === 0
                      ? 'No hay projectos en la papelera.'
            ...
            <DialogTitle>
              {confirm?.type === 'restore'
                ? confirm.ids.length === 1
                  ? 'Restaurar projecto'
                  : `Restaurar ${confirm.ids.length} projectos`
                : confirm?.ids.length === 1
                  ? 'Eliminar definitivamente'
                  : `Eliminar ${confirm?.ids.length} projectos`}
            </DialogTitle>
            <DialogDescription>
              {confirm?.type === 'restore'
                ? 'El projecto volverá a aparecer en tus listados activos.'
                : 'Esta acción no se puede deshacer. Se borrarán los datos guardados localmente de este projecto.'}
            </DialogDescription>
          </DialogHeader>
