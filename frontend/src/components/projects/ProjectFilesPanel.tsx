...
import {
  ...
  projectFileFromUpload,
  fileIconKind,
  getProjectFilePreviewUrl,
  isDemoProjectFilePreview,
  validateProjectFilesForUpload,
...
export function ProjectFilesPanel({
  ...
  const previewUrl = previewFile ? getProjectFilePreviewUrl(previewFile) : null
  ...
  const addFiles = useCallback(
    async (fileList: FileList | null) => {
      ...
      const validation = validateProjectFilesForUpload(files, incoming)
      if (validation) {
      ...
      try {
        ...
        for (const file of incoming) {
          uploaded.push(await projectFileFromUpload(file, authorName))
        }
        ...
        {files.length === 0 ? (
          ...
          <ul className="divide-y divide-border rounded-lg border border-border">
            {files.map((file) => {
              ...
              const canPreview = canPreviewProjectFile(file)
      ...
      >
        <DialogContent className="flex max-h-[92vh] flex-col gap-0 overflow-hidden p-0 sm:max-w-4xl">
          <DialogHeader className="border-b border-border px-6 py-4">
            ...
            <DialogDescription>
              {previewFile && isDemoProjectFilePreview(previewFile)
