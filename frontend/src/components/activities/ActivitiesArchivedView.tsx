...
import type { ArchivedActivityEntry } from '@/contexts/activities-registry-context'
...
import {
  ACTIVITY_ARCHIVE_RETENTION_DAYS,
  daysUntilPurge,
...
type ActivitiesArchivedViewProps = {
  query: string
}
...
function matchesQuery(entry: ArchivedActivityEntry, q: string): boolean {
  if (!q) return true
  const { activity } = entry
  return (
    activity.title.toLowerCase().includes(q) ||
    activity.relatedName.toLowerCase().includes(q) ||
    activity.assignee.toLowerCase().includes(q)
  )
}

export function ActivitiesArchivedView({ query }: ActivitiesArchivedViewProps) {
  const {
    archivedActivities,
    restoreActivity,
    restoreActivities,
  ...
  const { canDelete: canManageArchive } = useModulePermissions('actividades')
  ...
  const filtered = useMemo(
    () => archivedActivities.filter((e) => matchesQuery(e, q)),
    [archivedActivities, q],
  )
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (confirm.type === 'restore') {
        if (count === 1) await restoreActivity(confirm.ids[0]!)
        else await restoreActivities(confirm.ids)
        toast.success(
          count === 1
            ? 'Actividad restaurada correctamente.'
            : `${count} actividades restauradas.`,
        )
      } else {
        if (count === 1) await permanentlyDeleteActivity(confirm.ids[0]!)
        else await permanentlyDeleteActivities(confirm.ids)
        toast.success(
          count === 1
            ? 'Actividad eliminada definitivamente.'
            : `${count} actividades eliminadas definitivamente.`,
        )
      }
      setSelected(new Set())
      setConfirm(null)
      ...
      >
        ...
        <div className="space-y-1">
          ...
          <p className="text-amber-900/90 dark:text-amber-100/90">
            Los activityos archivados permanecen aquí durante{' '}
            <strong>{ACTIVITY_ARCHIVE_RETENTION_DAYS} días</strong>. Después se eliminan
          ...
          <table className="w-full min-w-[720px] border-collapse text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/30 text-left">
                ...
                <th className="px-4 py-3 font-semibold text-foreground">Activityo</th>
                ...
                <tr>
                  ...
                  >
                    {archivedActivities.length === 0
                      ? 'No hay activityos en la papelera.'
                        ...
                        <div className="flex items-center gap-3">
                          <div className="min-w-0">
                            ...
                            <p className="truncate text-xs text-muted-foreground">
                              {activity.relatedName} · {activity.due}
                            </p>
                          </div>
            ...
            <DialogTitle>
              {confirm?.type === 'restore'
                ? confirm.ids.length === 1
                  ? 'Restaurar activityo'
                  : `Restaurar ${confirm.ids.length} activityos`
                : confirm?.ids.length === 1
                  ? 'Eliminar definitivamente'
                  : `Eliminar ${confirm?.ids.length} activityos`}
            </DialogTitle>
            <DialogDescription>
              {confirm?.type === 'restore'
                ? 'El activityo volverá a aparecer en tus listados activos.'
                : 'Esta acción no se puede deshacer. Se borrarán los datos guardados localmente de este activityo.'}
            </DialogDescription>
          </DialogHeader>
