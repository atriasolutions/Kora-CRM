...
import { ActivitiesFiltersMenu } from '@/components/activities/ActivitiesFiltersMenu'
...
import {
  ACTIVITY_LIST_SCOPE_OPTIONS,
  ACTIVITY_SCOPE_SHORT_LABELS,
...
export type ActivitiesViewId = StandardModuleViewId

type ActivitiesModuleHeaderProps = {
  ...
  onListScopeChange?: (scope: ActivityListScope) => void
  archivedCount?: number
  toolbarEnd?: ReactNode
}

export function ActivitiesModuleHeader({
  view,
  onViewChange,
  query,
  onQueryChange,
  ...
  archivedCount = 0,
  toolbarEnd,
}: ActivitiesModuleHeaderProps) {
  const { canCreate } = useModulePermissions('actividades')
  const activeFilters = countActiveActivityFilters(filters)
      ...
      <div className="flex items-start justify-between gap-3 md:items-center">
        <div className="min-w-0">
          ...
          <p className="mt-1 hidden text-sm text-muted-foreground md:block">
            {view === 'archivados'
              ? 'Papelera de reciclaje: restaura o elimina actividades archivadas.'
              : 'Llamadas, reuniones, emails y tareas del equipo comercial.'}
            {activeFilters > 0
        ...
        <div className="flex shrink-0 items-center gap-2">
          {view !== 'archivados' ? (
            <ActivitiesFiltersMenu filters={filters} onFiltersChange={onFiltersChange} />
          ) : null}
      ...
      <ModuleListToolbarBar
        viewSwitcher={
          <ModuleViewSwitcher
            ...
            tablistAriaLabel="Vista de actividades"
        ...
        scopeSwitcher={
          showListScope ? (
            <ModuleListScopeSwitcher
              ...
              options={ACTIVITY_LIST_SCOPE_OPTIONS}
              shortLabels={ACTIVITY_SCOPE_SHORT_LABELS}
              showLabel
            />
          ) : undefined
        }
        search={
          <ModuleSearchField
            ...
            placeholder={
              view === 'archivados' ? 'Buscar en archivados…' : 'Buscar actividades…'
            }
