...
import {
  ...
  quoteFileFromUpload,
  fileIconKind,
  getQuoteFilePreviewUrl,
  isDemoQuoteFilePreview,
  validateQuoteFilesForUpload,
} from '@/lib/quote-files'
import { cn } from '@/lib/utils'

type QuoteFilesPanelProps = {
  authorName: string
  files: QuoteFile[]
  disabled?: boolean
...
export function QuoteFilesPanel({
  authorName,
  files,
  disabled = false,
  onFilesChange,
}: QuoteFilesPanelProps) {
  const inputId = useId()
  ...
  const previewUrl = previewFile ? getQuoteFilePreviewUrl(previewFile) : null
  ...
  const addFiles = useCallback(
    async (fileList: FileList | null) => {
      ...
      const validation = validateQuoteFilesForUpload(files, incoming)
      if (validation) {
      ...
      try {
        ...
        for (const file of incoming) {
          uploaded.push(await quoteFileFromUpload(file, authorName))
        }
      ...
      <CardContent className="space-y-4">
        ...
        {files.length === 0 ? (
          <p className="text-center text-sm text-muted-foreground">
            Aún no hay archivos asociados a este cotización.
          </p>
        ) : (
          <ul className="divide-y divide-border rounded-lg border border-border">
            {files.map((file) => {
              ...
              const canPreview = canPreviewQuoteFile(file)
      ...
      >
        <DialogContent className="flex max-h-[92vh] flex-col gap-0 overflow-hidden p-0 sm:max-w-4xl">
          <DialogHeader className="border-b border-border px-6 py-4">
            ...
            <DialogDescription>
              {previewFile && isDemoQuoteFilePreview(previewFile)
