...
import type { ArchivedQuoteEntry } from '@/contexts/quotes-registry-context'
...
function matchesQuery(entry: ArchivedQuoteEntry, q: string): boolean {
  if (!q) return true
  const { quote } = entry
  return (
...
export function QuotesArchivedView({ query }: QuotesArchivedViewProps) {
  const {
    archivedQuotes,
    restoreQuote,
    restoreQuotes,
  ...
  const { canDelete: canManageArchive } = useModulePermissions('cotizaciones')
  ...
  const filtered = useMemo(
    () => archivedQuotes.filter((e) => matchesQuery(e, q)),
    [archivedQuotes, q],
  )
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (confirm.type === 'restore') {
        if (count === 1) await restoreQuote(confirm.ids[0]!)
        else await restoreQuotes(confirm.ids)
        toast.success(
          count === 1
            ? 'Cotización restaurada correctamente.'
            : `${count} cotizaciones restauradas.`,
        )
      } else {
        if (count === 1) await permanentlyDeleteQuote(confirm.ids[0]!)
        else await permanentlyDeleteQuotes(confirm.ids)
        toast.success(
          count === 1
            ? 'Cotización eliminada definitivamente.'
            : `${count} cotizaciones eliminadas definitivamente.`,
        )
      }
      setSelected(new Set())
      setConfirm(null)
      ...
      >
        ...
        <div className="space-y-1">
          ...
          <p className="text-amber-900/90 dark:text-amber-100/90">
            Las cotizaciones archivadas permanecen aquí durante{' '}
            <strong>{QUOTE_ARCHIVE_RETENTION_DAYS} días</strong>. Después se eliminan de forma
            definitiva. Restaura las que aún necesites o elimínalas manualmente antes de esa fecha.
          </p>
        </div>
      </div>
          ...
          <table className="w-full min-w-[720px] border-collapse text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/30 text-left">
                ...
                <th className="px-4 py-3 font-semibold text-foreground">Cotización</th>
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="min-w-0 space-y-1">
                          <p className="font-mono font-semibold text-foreground">{quote.code}</p>
                          <p className="truncate text-sm text-muted-foreground">{quote.title}</p>
                          <p className="text-xs text-muted-foreground">
                            {quote.companyName} · {quote.opportunityName}
                          </p>
            ...
            <DialogTitle>
              {confirm?.type === 'restore'
                ? confirm.ids.length === 1
                  ...
                  : `Restaurar ${confirm.ids.length} cotizaciones`
                : confirm?.ids.length === 1
                  ? 'Eliminar definitivamente'
                  : `Eliminar ${confirm?.ids.length} cotizaciones`}
            </DialogTitle>
            <DialogDescription>
              {confirm?.type === 'restore'
                ? 'La cotización volverá a aparecer en tus listados activos.'
                : 'Esta acción no se puede deshacer. La cotización se eliminará del servidor y dejará de aparecer en archivados.'}
            </DialogDescription>
          </DialogHeader>
