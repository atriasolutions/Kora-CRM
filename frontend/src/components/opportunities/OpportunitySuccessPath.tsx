import { ArrowLeft, ArrowRight, Check, ChevronRight, Pause, Target } from 'lucide-react'
...
import type { OpportunityStageHistory } from '@/lib/opportunity-journey'
import {
  OPPORTUNITY_JOURNEY_MAIN_LINE,
  OPPORTUNITY_JOURNEY_OFF_ROUTE,
  OPPORTUNITY_OFF_ROUTE_ENTRY_MAIN_STAGES,
...
type OpportunitySuccessPathProps = {
  currentStage: OpportunityJourneyStage
  history?: OpportunityStageHistory[]
  readOnly?: boolean
  onStageChange?: (stage: OpportunityJourneyStage) => void
  className?: string
}
...
function shortMainLabel(stage: string) {
  if (stage === 'En diagnóstico') return 'Diagnóstico'
  if (stage === 'Negociación') return 'Negociación'
  return stage
}

function shortOffRouteLabel(stage: string) {
  if (stage === 'En espera cliente') return 'Espera cliente'
  if (stage === 'Pausada internamente') return 'Pausa interna'
  if (stage === 'No calificada') return 'No calif.'
  return stage
}

export function OpportunitySuccessPath({
  currentStage,
  history = [],
  readOnly = false,
  onStageChange,
  className,
}: OpportunitySuccessPathProps) {
  ...
  const pausedFromIndex = OPPORTUNITY_JOURNEY_MAIN_LINE.indexOf(pausedFrom)
  const mainIndex = OPPORTUNITY_JOURNEY_MAIN_LINE.indexOf(
    currentStage as (typeof OPPORTUNITY_JOURNEY_MAIN_LINE)[number],
  )
  ...
  const progressPct =
    OPPORTUNITY_JOURNEY_MAIN_LINE.length > 1
      ? (progressIndex / (OPPORTUNITY_JOURNEY_MAIN_LINE.length - 1)) * 100
      : 0
  ...
  const showOffRoutePanel = currentStage !== 'Cerrada'
  const previousMain =
  ...
  return (
    <section
      ...
      aria-label="Ruta del éxito de la oportunidad"
    >
      ...
      <div className="space-y-4 p-4 sm:p-5">
        <div className="relative mx-auto max-w-4xl">
          ...
          <ol className="relative flex justify-between gap-0.5">
            {OPPORTUNITY_JOURNEY_MAIN_LINE.map((stage, index) => {
        ...
        {showOffRoutePanel ? (
          ...
          >
              ...
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-muted-foreground">
                    Pausas, pérdida o descarte. Disponible desde{' '}
                    <span className="font-medium text-foreground">
                      {OPPORTUNITY_OFF_ROUTE_ENTRY_MAIN_STAGES.map(shortMainLabel).join(', ')}
                    </span>
            ...
            >
              {OPPORTUNITY_JOURNEY_OFF_ROUTE.map((stage) => {
            ...
            {!onOffRoute && offRouteTransitions.length === 0 ? (
              <p className="mt-3 text-xs text-muted-foreground">
                Avanza al menos a{' '}
                <strong className="font-medium text-foreground">En diagnóstico</strong> para
