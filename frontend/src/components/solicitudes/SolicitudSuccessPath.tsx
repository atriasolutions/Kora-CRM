import { ArrowLeft, ArrowRight, Check, ChevronRight, ClipboardList, Pause } from 'lucide-react'
...
import {
  ...
  SOLICITUD_STOPPER_ENTRY_MAIN_STAGES,
...
type SolicitudSuccessPathProps = {
  currentStage: SolicitudJourneyStage
  history?: SolicitudStatusHistoryEntry[]
  readOnly?: boolean
  onStageChange?: (stage: SolicitudJourneyStage) => void
  className?: string
}
...
function shortMainLabel(stage: string) {
  ...
  if (stage === 'Planificación') return 'Planif.'
  return stage
}
...
function shortStopperLabel(stage: string) {
  ...
  if (stage === 'En espera de Cliente') return 'Espera cliente'
  return stage
}

export function SolicitudSuccessPath({
  currentStage,
  history = [],
  readOnly = false,
  onStageChange,
  className,
}: SolicitudSuccessPathProps) {
  ...
  const pausedFromIndex = SOLICITUD_JOURNEY_MAIN_LINE.indexOf(pausedFrom)
  const mainIndex = SOLICITUD_JOURNEY_MAIN_LINE.indexOf(
    currentStage as (typeof SOLICITUD_JOURNEY_MAIN_LINE)[number],
  )
  ...
  const progressPct =
    SOLICITUD_JOURNEY_MAIN_LINE.length > 1
      ? (progressIndex / (SOLICITUD_JOURNEY_MAIN_LINE.length - 1)) * 100
      : 0
  ...
  const mainStepState = (
    stage: (typeof SOLICITUD_JOURNEY_MAIN_LINE)[number],
    index: number,
  ): MainStepState => {
  ...
  return (
    <section
      ...
      aria-label="Ruta del éxito de la solicitud"
    >
      ...
      <div className="space-y-4 p-3 sm:p-4 md:p-5">
        <div className="-mx-3 overflow-x-auto px-3 sm:mx-0 sm:px-0 md:overflow-visible">
          <div className="relative mx-auto min-w-[640px] max-w-4xl md:min-w-0">
            ...
            <ol className="relative flex justify-between gap-0.5">
              {SOLICITUD_JOURNEY_MAIN_LINE.map((stage, index) => {
        ...
        {showStopperPanel ? (
          ...
          >
              ...
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-muted-foreground">
                    Detención temporal o en espera. Disponible desde{' '}
                    <span className="font-medium text-foreground">
                      {SOLICITUD_STOPPER_ENTRY_MAIN_STAGES.map(shortMainLabel).join(', ')}
                    </span>
            ...
            >
              {SOLICITUD_JOURNEY_STOPPERS.map((stage) => {
            ...
            {!onStopper && stopperTransitions.length === 0 ? (
              <p className="mt-3 text-xs text-muted-foreground">
                Avanza al menos a{' '}
                <strong className="font-medium text-foreground">Planificación</strong> para registrar
                un estado fuera de ruta.
              </p>
            ) : null}
          </aside>
        ) : null}
