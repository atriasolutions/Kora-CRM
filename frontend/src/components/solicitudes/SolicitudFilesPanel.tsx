...
export function SolicitudFilesPanel({
  ...
  const addFiles = useCallback(
    async (fileList: FileList | null) => {
      ...
      const validation = validateSolicitudFilesForUpload(files, incoming)
      if (validation) {
      ...
      setUploading(true)
      try {
        const uploaded: SolicitudFile[] = []
        for (const file of incoming) {
          uploaded.push(await solicitudFileFromUpload(file, authorName))
        }
  ...
  return (
    <Card className="shadow-sm">
      ...
      <CardContent className="space-y-4">
        ...
        >
          <Upload aria-hidden className="mb-2 size-8 text-muted-foreground" />
        ...
        {files.length === 0 ? (
          <p className="text-center text-sm text-muted-foreground">
            Aún no hay archivos asociados a esta solicitud.
          </p>
        ) : (
          <ul className="divide-y divide-border rounded-lg border border-border">
            {files.map((file) => {
              ...
              const canPreview = canPreviewSolicitudFile(file)
                ...
                >
                  ...
                  <div className="flex shrink-0 items-center gap-1 sm:ms-2">
                    {canPreview ? (
                      ...
                      >
                        {isImageEntityFile(file) ? 'Ver imagen' : 'Ver PDF'}
                      </Button>
                    ) : null}
                    {canDownload ? (
                      <Button
      ...
      <EntityFilePreviewDialog
        file={previewFile}
        open={previewFile !== null && canPreviewSolicitudFile(previewFile)}
        onOpenChange={(open) => {
