...
import type { ArchivedSolicitudEntry } from '@/contexts/solicitudes-registry-context'
...
import {
  SOLICITUD_ARCHIVE_RETENTION_DAYS,
  daysUntilPurge,
...
type SolicitudesArchivedViewProps = {
  query: string
}
...
function matchesQuery(entry: ArchivedSolicitudEntry, q: string): boolean {
  if (!q) return true
  const { solicitud } = entry
  return (
    solicitud.title.toLowerCase().includes(q) ||
    solicitud.code.toLowerCase().includes(q) ||
    solicitud.assignee.toLowerCase().includes(q)
  )
}

export function SolicitudesArchivedView({ query }: SolicitudesArchivedViewProps) {
  const {
    archivedSolicitudes,
    restoreSolicitud,
    restoreSolicitudes,
  ...
  const { canDelete: canManageArchive } = useModulePermissions('solicitudes')
  ...
  const filtered = useMemo(
    () => archivedSolicitudes.filter((e) => matchesQuery(e, q)),
    [archivedSolicitudes, q],
  )
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (confirm.type === 'restore') {
        if (count === 1) await restoreSolicitud(confirm.ids[0]!)
        else await restoreSolicitudes(confirm.ids)
        toast.success(
          count === 1
            ? 'Solicitud restaurada correctamente.'
            : `${count} solicitudes restauradas.`,
        )
      } else {
        if (count === 1) await permanentlyDeleteSolicitud(confirm.ids[0]!)
        else await permanentlyDeleteSolicitudes(confirm.ids)
        toast.success(
          count === 1
            ? 'Solicitud eliminada definitivamente.'
            : `${count} solicitudes eliminadas definitivamente.`,
        )
      }
      setSelected(new Set())
      setConfirm(null)
      ...
      >
        ...
        <div className="space-y-1">
          ...
          <p className="text-amber-900/90 dark:text-amber-100/90">
            Las solicitudes archivadas permanecen aquí durante{' '}
            <strong>{SOLICITUD_ARCHIVE_RETENTION_DAYS} días</strong>. Después se eliminan
            de forma definitiva y no podrás recuperarlas.
          </p>
        </div>
      </div>
          ...
          <table className="w-full min-w-[720px] border-collapse text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/30 text-left">
                ...
                <th className="px-4 py-3 font-semibold text-foreground">Solicitud</th>
                  ...
                  return (
                    ...
                    >
                      <td className="px-4 py-3">
                        <input
                          aria-label={`Seleccionar ${solicitud.title}`}
                        ...
                        <div className="flex items-center gap-3">
                          <div className="min-w-0">
                            ...
                            <p className="truncate text-xs text-muted-foreground">
                              {solicitud.code} · {solicitud.assignee || '—'}
                            </p>
                          </div>
            ...
            <DialogTitle>
              {confirm?.type === 'restore'
                ? confirm.ids.length === 1
                  ? 'Restaurar solicitud'
                  : `Restaurar ${confirm.ids.length} solicitudes`
                : confirm?.ids.length === 1
                  ? 'Eliminar definitivamente'
                  : `Eliminar ${confirm?.ids.length ?? 0} solicitudes`}
            </DialogTitle>
            <DialogDescription>
              {confirm?.type === 'restore'
                ? 'La solicitud volverá a aparecer en tus listados activos.'
                : 'Esta acción no se puede deshacer. Se borrarán los datos guardados localmente de esta solicitud.'}
            </DialogDescription>
          </DialogHeader>
