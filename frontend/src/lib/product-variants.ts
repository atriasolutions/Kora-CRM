...
export function normalizeVariantOptions(raw: unknown): VariantOption[] {
...
export function normalizeVariantAttributes(raw: unknown): VariantAttributes {
...
export function suggestVariantSku(
  ...
  const parts = keys
    ...
    .filter(Boolean)
  return [base, ...parts].join('-').slice(0, 64)
}
...
export function resolveVariantKind(input: {
  ...
  if (input.variantKind) return input.variantKind
...
export function formatAttributesChips(
